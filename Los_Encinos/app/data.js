 ...
 * Handles GeoJSON processing, localStorage syncing, and attribute mapping.
 */
...
const DataModule = (() => {
    ...
    function init() {
        ...
        if (saved) {
            ...
            syncStatus(window.json_Reservadas_5, 'Reservada');
            save();
        } else {
            // Merge raw GeoJSON from qgis2web global variables
    ...
    function processBatch(geoJson, defaultEstado) {
        ...
        geoJson.features.forEach(f => {
            // Find best property match for ID
            ...
            const normalizedFeature = {
                ...
                geometry: f.geometry,
                properties: {
                    ...f.properties, // Keep original data
                    id_lote: id_lote,
                    area: area,
                    estado: estado,
    ...
    // ── Sync Queue (Local Persistence for Offline) ──
